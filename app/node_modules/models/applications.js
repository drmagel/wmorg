...
var collectionName = 'applications',
  ...
  this.getUserAssets = function(userID, assets, filter, callback) {
    var userID = Number(userID)
      , assets = assets || []
      , query = assets.length ? {$and: [{userID:userID},{type:'asset'},{appID: {$in: assets.map(function(el){return Number(el)})}}]}
                              : {$and: [{userID:userID},{type: 'asset'}]}
      ;
    filter ? filter._id = 0 : filter = {_id: 0};
    return this.collection.find(query, filter)
               .toArray(callback || function(){});
