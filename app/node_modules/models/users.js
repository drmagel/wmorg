...
*/
  ...
  this.checkPhone = function(phone, callback) {
    return this.collection.find(
             {phone: phone},
             {_id:0, userID:1, phone:1, userStatus:1, groupAdminUserID:1}
           ).toArray(callback);
  };
...
// For the test purpose only
  ...
  this.getAllUsers = function(query, filter, callback) {
    var query = query || {};
    filter ? filter._id = 0 : filter = {_id: 0};
    return this.collection.find(query,filter).toArray(callback);
  };
//************************************************************************
